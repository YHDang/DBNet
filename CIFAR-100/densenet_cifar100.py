...
iterations = 391
weight_decay = 1e-4
model_path = 'cifar-100-model/densenet_cifar100_model.h5'
flag = args.mode
...
def densenet_train(model, x_train, y_train, x_test, y_test):
    ...
    ckpt = ParallelModelCheckpoint(model, model_path, monitor='val_loss', save_best_only=True, save_weights_only=True, mode='auto', period=1)
...
def bls_dense_train(model, train_x, train_y):
    ...
    feature_layer.save('cifar-100-model/densenet_feature_layer_model.h5')
...
def bls_dense_test(x_test, y_test, weightOfEnhanceLayer, parameterOfShrink, OutputWeight):
    ...
    feature_layer = load_model('cifar-100-model/densenet_feature_layer_model.h5')
...
def run_main():
    ...
    if flag == 'train':
        ...
        np.savetxt('result/test_acc_5000.txt', test_acc)
        ...
        for i in range(L):
            ...
            n3 = n3 + m
        np.savetxt('result/densenet/enhance_num_v2.txt', enhance_num)
        np.savetxt('result/densenet/test_acc_v2.txt', Test_Acc)
        np.savetxt('result/densenet/dense_test_time_v2.txt', Test_Time)
