...
parses.add_argument('--mode', '-m', default='test', help='Select running mode: train, test and bls')
...
model_path = 'cifar-100-model/vgg19_cifar100_model.h5'
...
def build_vgg19_model(input_shape):
    ...
    x = Dense(num_classes, kernel_regularizer=keras.regularizers.l2(weight_decay), kernel_initializer=he_normal(),
                name='predictions_cifa100', activation='softmax')(x)

    model = Model(inputs=inputs, outputs=x)
    return model


def vgg19_train(model, x_train, y_train, x_test, y_test):
    ...
    change_lr = keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.1, verbose=0, mode='min', epsilon=0.00001, patience=50)
...
def bls_vgg_train(model, train_x, train_y):
    ...
    model.load_weights(model_path)
    model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
    feature_layer = Model(inputs=model.input, outputs=model.get_layer('activation_18').output)
...
def run_main():
    ...
    if flag == 'train':
        vgg19_train(_model, x_train, y_train, x_test, y_test)
    elif flag == 'test':
        ...
        np.savetxt('result/vgg/vgg_test_acc.txt', Test_Acc)
