...
model_path = 'cifar-10-model/alexnet_cifar10_model.h5'
flag = args.mode
...
def bls_vgg_train(model, train_x, train_y):
    ...
    feature_layer.save('cifar-10-model/alexnet_feature_layer_model.h5')
    ...
    for i in range(N2):
        ...
        weightOfEachWindow = 2 * random.randn(FeatureOfEachWindow.shape[1] + 1, N1) - 1
        FeatureOfInputDataWithBias = np.hstack([FeatureOfEachWindow, np.ones((train_x.shape[0], 1))])
#        FeatureOfEachWindow = feature_layer.predict(train_x)
...
def bls_vgg_test(x_test, y_test, weightOfEnhanceLayer, parameterOfShrink, OutputWeight):
    ...
    feature_layer = load_model('cifar-10-model/alexnet_feature_layer_model.h5')
...
def run_main():
    ...
    if flag == 'train':
        ...
        os.environ['CUDA_VISBILE_DEVICES'] = '0, 1'
        weightOfEnhanceLayer, parameterOfShrink, OutputWeight, InputOfEnhanceLayerWithBias, InputOfOutputLayer, pinvOfInput = bls_vgg_train(
        ...
        for i in range(L):
 #           test_acc, test_time, train_acc, train_time = BLS_CNN(_model, model_path, x_train, y_Train, x_test, y_Test, s, c, 10, 10, n3)
 #           test_acc, test_time, train_acc, train_time = BLS_AlexNet(_model, model_path, x_train, y_train_label, x_test, y_test_label, s, c, 10, 10, n3)
            ...
            test_acc, test_time, train_acc, train_time = BLS_AlexNet(_model, model_path, x_train,
                                                                     y_train_label, x_test, y_test_label, s, c, 10, 10, n3)
            Test_Acc.append(test_acc)
            Test_Time.append(test_time)
            enhance_num.append(n3)
            n3 = n3 + m

        np.savetxt('result/alexnet/alexnet_bls_test_acc.txt', Test_Acc)
        np.savetxt('result/alexnet/alexnet_bls_test_time.txt', Test_Time)
        np.savetxt('result/alexnet/enhancement_node_num.txt', enhance_num)            
#    BLS_AlexNet(_model, model_path, x_train, y_train_label, x_test, y_test_label, s, c, 10, 10, 500)
